...
class Picasa {  
    ...
    getPhotos( album, photos ) {
        if( album.num_photos<=0 ) {
            ...
            return Promise.all(queries.map( query => this.get_photos_by_query(album,photos,query).catch( (err) => {
